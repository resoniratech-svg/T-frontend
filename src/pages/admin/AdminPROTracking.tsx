...
export default function AdminPROTracking() {
  ...
  const inactiveEmployeesCount = useMemo(() => {
    return sectorFilteredContracts.filter((c: any) => c && c.status === "Inactive").length;
  }, [sectorFilteredContracts]);
    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
        ...
        <StatCard
          ...
          value={inactiveEmployeesCount.toString()}
          icon={<ShieldAlert className="text-slate-500" size={20} />}
          trend={{ value: "Inactive / Suspended", positive: false }}
        />
        <StatCard
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4 flex-1">
          ...
          <div className="flex flex-col gap-1 flex-1 max-w-xs">
            ...
            <div className="w-full bg-slate-100 border border-slate-200 rounded-lg px-4 py-2 text-sm font-bold text-slate-500 min-h-[38px] flex items-center italic">
              {selectedClient === "All" ? "Select a client to manage employees" : selectedClient === "Admin" ? "Admin" : (safeClients.find(c => c.id.toString() === selectedClient.toString())?.name || selectedClient)}
            </div>
          </div>
        </div>
        ...
        {selectedClient !== "All" && (
          <button
            onClick={() => {
              if (selectedClient === "Admin") {
                navigate(`/admin-employees/create?from=pro-tracking${activeDivision !== "all" ? `&division=${activeDivision}` : ""}`);
              } else {
          ...
          >
            <Users size={14} /> Add Employee to {selectedClient === "Admin" ? "Admin" : (safeClients.find(c => c.id.toString() === selectedClient.toString())?.contactPerson || selectedClient)}
          </button>
        )}
      </div>
                      ...
                      <tr key={contract.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex items-center justify-end gap-1">
                            <button
                              onClick={() => {
                                if (contract.id.startsWith("ADM-")) {
                                  navigate(`/admin-employees/edit/${contract.id}`);
                                } else {
                                  navigate(`/employees/edit/${contract.id}`);
                                }
                              }}
                            ...
                            <button
                              onClick={() => {
                                if (contract.id.startsWith("ADM-")) {
                                  navigate(`/admin-employees/details/${contract.id}`);
                                } else {
                                  navigate(`/employees/details/${contract.id}`);
                                }
                              }}
