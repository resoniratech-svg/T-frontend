...
export default function AddEditAdminEmployee() {
  ...
  const { data: employee } = useQuery<Employee>({
    ...
    queryFn: () => adminEmployeeService.getEmployee(id!),
    enabled: isEdit && !!id
  });
  ...
  useEffect(() => {
    if (employee) {
      setFormData(employee);
        ...
        // Generate a temporary ID for new admin employees
  ...
  const mutation = useMutation({
    mutationFn: (data: Partial<Employee>) => isEdit ? adminEmployeeService.updateEmployee(id!, data) : adminEmployeeService.createEmployee(data),
    onSuccess: async (data) => {
        console.log("Admin employee created/updated successfully:", data);
        await Promise.all([
            queryClient.invalidateQueries({ queryKey: ["admin-employees"] }),
            queryClient.invalidateQueries({ queryKey: ["pro-contracts"] }),
            queryClient.invalidateQueries({ queryKey: ["pro-all-documents"] }),
        ]);
        if (isEdit) await queryClient.invalidateQueries({ queryKey: ["admin-employee", id] });
        
        alert(`Admin employee ${isEdit ? 'updated' : 'registered'} successfully!`);
    ...
    onError: (error: Error) => {
        alert(`Failed to ${isEdit ? 'update' : 'create'} admin employee: ${error.message || 'Unknown error'}`);
    }
  });
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-20 font-sans">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">{isEdit ? "Edit Admin Employee" : "Add New Admin Employee"}</h1>
          <p className="text-xs sm:text-sm text-gray-500">{isEdit ? `Modifying details for ${formData.name}` : "Enter admin staff details to register in EMS"}</p>
        </div>
      </div>
        ...
        {step === 1 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  readOnly
                  disabled
                  className="w-full px-4 py-2.5 bg-gray-100 border border-gray-200 rounded-lg text-sm text-gray-500 outline-none font-bold"
                />
              </div>
          ...
          <div className="space-y-8 animate-fade-in font-sans">
            ...
            <div className="flex justify-between items-center pt-8 border-t border-gray-100">
              ...
              >
                <Check size={18} /> {isEdit ? "Update Admin Employee" : "Complete Registration"}
              </button>
            </div>
          </div>
        )}
