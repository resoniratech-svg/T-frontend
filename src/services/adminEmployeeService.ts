import api from "./api";
import type { Employee } from "../types/employee";

export const adminEmployeeService = {
  getEmployees: async (division?: string): Promise<Employee[]> => {
    const { data } = await api.get("/admin-employees", { params: { division } });
    return data.data || data;
  },

  getEmployee: async (id: string): Promise<Employee> => {
    const { data } = await api.get(`/admin-employees/${id}`);
    return data.data || data;
  },

  createEmployee: async (data: Partial<Employee>): Promise<Employee> => {
    const { data: resData } = await api.post("/admin-employees", data);
    return resData.data || resData;
  },

  updateEmployee: async (id: string, data: Partial<Employee>): Promise<Employee> => {
    const { data: resData } = await api.put(`/admin-employees/${id}`, data);
    return resData.data || resData;
  },

  deleteEmployee: async (id: string): Promise<{ success: boolean }> => {
    const { data } = await api.delete(`/admin-employees/${id}`);
    return data.data || data;
  }
};
export default adminEmployeeService;
